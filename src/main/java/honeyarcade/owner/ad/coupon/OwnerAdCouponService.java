...
public class OwnerAdCouponService {
	...
	public void reqProc(HashMap dataMap) throws Exception {
		...
		OwnerAdCouponVO paramVO = new OwnerAdCouponVO();
		...
		//	광고 요청한 데이터가 있는지 확인
		OwnerAdCouponVO adVO = ownerAdCouponMapper.getAdReq(paramVO);
		//	지역광고 요청
		if(adVO == null) {
			//	데이터가 없는 경우 인서트
			ownerAdCouponMapper.insertAdReq(paramVO);
		}else {
			//	데이터가 있는 경우 업데이트
			...
			ownerAdCouponMapper.updateAdReq(paramVO);
		}
		//	쿠폰 사용 체크
		ownerAdCouponMapper.updatePayDtl(paramVO);
